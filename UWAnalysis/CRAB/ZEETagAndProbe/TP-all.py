...
addTagAndProbePlotter(process,'ElectronTagAndProbePlotter',
                              ...
                              ['ID','SIP','ISO'],
                              ['(abs(superCluster().eta())<0.8 && electronID("mvaNonTrigV0")>0.5) || (abs(superCluster().eta())<1.479 && abs(superCluster().eta())>0.8 && electronID("mvaNonTrigV0")>0.12) ||  (abs(eta())>1.479 && electronID("mvaNonTrigV0")>0.6)','abs(userFloat("ip3DS"))<4','(chargedHadronIso()+max(0.0,neutralHadronIso()+photonIso()-userFloat("zzRho2012")*userFloat("EAGammaNeuHadron04")))/pt()<0.40'],
