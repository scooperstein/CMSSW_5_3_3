...
ETanalysisConfigurator.addSmearing('triggeredPatTaus','triggeredPatMuons','triggeredPatElectrons','cleanPatJets','patMVAMetEleTau','ET')
...
#Make DiTaus
ETanalysisConfigurator.addDiCandidateModule('eleTaus','PATEleTauPairProducer','smearedElectronsET','smearedTausET','smearedMETET','smearedJetsET',1,9999,text = 'AtLeastOneEleTau',leadingObjectsOnly = False,dR = 0.5,recoMode = "",genParticles='genDaughters')
...
ETanalysisConfigurator.addSelector('eleTausEleConvRej','PATEleTauPairSelector','!(leg1.userFloat("hasConversion")>0)&&leg1.userInt("missingHits")==0','electronConvRej',1)
...
ETanalysisConfigurator.addSelector('eleTausVLooseIsolation','PATEleTauPairSelector','leg2.tauID("byIsolationMVAraw")>0.7','ETTauVLooseMVAIso',1)
ETanalysisConfigurator.addSelector('eleTausTauMuonVeto','PATEleTauPairSelector','leg2.tauID("againstMuonLoose")','ETAgainstMuon',1)
ETanalysisConfigurator.addSelector('eleTausTauElectronVeto','PATEleTauPairSelector','leg2.tauID("againstElectronMedium")&&leg2.tauID("againstElectronMVA")','ETAgainstElectron',1)
...
ETanalysisConfigurator.addSelector('eleTausLooseIsolation','PATEleTauPairSelector','leg2.tauID("byLooseIsolationMVA")>0','ETTauLooseMVAIso',1)
...
#Add smearing
MTanalysisConfigurator.addSmearing('triggeredPatTaus','triggeredPatMuons','triggeredPatElectrons','cleanPatJets','patMVAMetMuTau','MT')
...
MTanalysisConfigurator.addSelector('osDiMuons','PATMuPairSelector','abs(leg1.userFloat("dz"))<0.2&&abs(leg2.userFloat("dz"))<0.2&&charge==0&&(!leg1.pfCandidateRef().isNull())&&(!leg2.pfCandidateRef().isNull())&&leg1.isTrackerMuon&&leg2.isTrackerMuon&&leg1.isGlobalMuon&&leg2.isGlobalMuon&&leg1.pt()>15&&leg2.pt()>15&& (leg1.userIso(0)+max(leg1.photonIso+leg1.neutralHadronIso()-0.5*leg1.puChargedHadronIso,0.0))/leg1.pt()<0.3 &&(leg2.userIso(0)+max(leg2.photonIso+leg2.neutralHadronIso()-0.5*leg2.puChargedHadronIso,0.0))/leg2.pt()<0.3','DiMuonCreation',0,100)
MTanalysisConfigurator.addSorter('diMuonsSorted','PATMuPairSorter')

#Make DiTaus
MTanalysisConfigurator.addDiCandidateModule('diTaus','PATMuTauPairProducer','smearedMuonsMT','smearedTausMT','smearedMETMT','smearedJetsMT',1,9999,text='AtLeastOneDiTau',leadingObjectsOnly = False,dR = 0.5,recoMode ="",genParticles='genDaughters')
...
MTanalysisConfigurator.addSelector('diTausVLooseTauIsolation','PATMuTauPairSelector','leg2.tauID("byIsolationMVAraw")>0.7','MTTauVLooseMVAIso',1)
...
MTanalysisConfigurator.addSelector('diTausTauMuonVeto','PATMuTauPairSelector','leg2.tauID("againstMuonTight")','MTAgainstMuon',1)
...
MTanalysisConfigurator.addSelector('diTausLooseTauIsolation','PATMuTauPairSelector','leg2.tauID("byLooseIsolationMVA")>0','MTTauLooseMVAIso',1)
