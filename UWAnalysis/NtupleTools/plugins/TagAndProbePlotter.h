...
class TagAndProbePlotter : public edm::EDAnalyzer {
  ...
  explicit TagAndProbePlotter(const edm::ParameterSet& iConfig):
	...
	for(unsigned int i=0;i<selections_.size();++i) {
	  ...
	  ///////
          t->Branch(("BXminus"),&value[0],("BXminus/F"));
          t->Branch(("BX0"),&value[1],("BX0/F"));
          t->Branch(("BXplus"),&value[2],("BXplus/F"));
	  ///////
	}
	...
	t->Branch("massHi",&massHi,"massHi/F");
	t->Branch("massLo",&massLo,"massLo/F");
   ...
   private:
	...
	if(iEvent.getByLabel(edm::InputTag("addPileupInfo"), PupInfo)) {
	  for(std::vector<PileupSummaryInfo>::const_iterator i = PupInfo->begin();
	    ...
	    if(BX==0) {
	      value[1] =  i->getPU_NumInteractions();
	    }
	    if(BX==1) {
...
/*  	      printf("Paths = %s\n",paths->at(j).name().c_str());  */
	...
	for(unsigned int i=0;i<1;++i) {
	    ...
	    mass=ref->at(i).mass();
	    massHi=(1.02*(ref->at(i).leg1()->p4())+1.02*(ref->at(i).leg2()->p4())).mass();
	    massLo=(0.98*(ref->at(i).leg1()->p4())+0.98*(ref->at(i).leg2()->p4())).mass();
	    pt=ref->at(i).leg2()->pt();
