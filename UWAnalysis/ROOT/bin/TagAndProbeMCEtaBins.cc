...
//	lumi = 4402.0;  //DYmumu lumi
  ...
  TFile *graphs = new TFile(TString::Format("graphs-%s.root",varname.c_str()),"RECREATE");
  graphs->cd();
  ...
  // plot Data/MC efficiencies
    ...
    gd->Write(TString::Format("data_efficiencies_eta_%s_to_%s",etaLos[i].c_str(),etaHis[i].c_str())); 
    gm->Write(TString::Format("mc_efficiencies_eta_%s_to_%s",etaLos[i].c_str(),etaHis[i].c_str()));
    ...
    cFactors->Write(TString::Format("correction_factors_eta__%s_to_%s",etaLos[i].c_str(),etaHis[i].c_str()));
  }
...
void run(const std::string& pt_lo,const std::string& pt_hi,const std::string& eta_lo,const std::string& eta_hi,const std::string& savefile,float factor,const std::string& dirMC,const std::string& dirData, const std::string& inTree,const std::string& cut, const std::string& var,FILE *effTex,const std::string& signal){
  ...
  cutPassBarrel = "("+cut+"matchPass==1&&(eta>"+eta_lo+"&&eta<"+eta_hi+")&&("+passCut+")&&pt>"+pt_lo+"&&pt<"+pt_hi+")";
  cutFailBarrel = "("+cut+"matchPass==1&&(eta>"+eta_lo+"&&eta<"+eta_hi+")&&("+failCut+")&&pt>"+pt_lo+"&&pt<"+pt_hi+")";
  ...
  if(string(signal.c_str()).compare("DYee") == 0){
  	w->factory(TString::Format("nExp[%f,0,1000000000]",(bkgrdYield)+10));
       printf("It's DYee!!\n");
       printf("Signal: %s\n",signal.c_str());
  }
  else{
	w->factory(TString::Format("nExp[%f,0,1000000000.0]",bkgrdYield*2+10));
        printf("It's DYumu!!\n");
  }
  ...
  if (bkgrdFailYield != 0) {
    w->factory(TString::Format("nExpFail[%f,0,%f]",2*bkgrdFailYield,bkgrdFailYield*1000));
  }
 ...
 // }
  ...
  else w->factory(TString::Format("FCONV::SigGausFail(mass,%s,gaussFail)",signal.c_str())) ;
